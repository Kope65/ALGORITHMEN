...
public class Hashtable<T> implements Map {
	 ...
	 */
	public int getElementCount() { 												// Rufe Zaehler auf
		int counter = 0;
		for (int i = 0; i < hashtable.length; i++) {
		...
		return counter; 														// Gebe Zaehler aus
	}
	 ...
	 */
	public LinkedList<KeyValuePair>[] checkAndResizeHashtable(LinkedList<KeyValuePair>[] hashtableToCheck) { //ueberpruefe und vergroessere Hashtable
		if((hashtableToCheck.length/2) <= getElementCount()) {												 
			LinkedList<KeyValuePair>[] helperHashtable = new LinkedList[hashtableToCheck.length * 2];		 //fuegt KeyValuePair in Hilfstabelle ein
			...
			for (int i = 0; i < hashtableToCheck.length; i++) {
				if(!hashtableToCheck[i].isEmpty()) {														 //falls i leer...
					for (int j = 0; j < hashtableToCheck[i].size(); j++) {									 
						if(!hashtableToCheck[i].get(j).equals(null)) {										 //...vergleiche Eintraege
							double helperIndexDouble = Math.ceil(hashtableToCheck[i].get(j).getKey().hashCode() % helperHashtable.length); //ermittle den helperIndex zum 'key'	als 'double'	
							int helperIndex = (int) Math.ceil(helperIndexDouble);							 //Runde den 'double' auf
							if(helperIndex < 0) {
								helperIndex *= -1;															 //wenn negativ, multipliziere 'helperIndex' mit -1
							}
			...
			return helperHashtable; // gibt Eintrag aus 'helperHashtable' aus
		}
		return hashtableToCheck; // gibt Eintrag aus 'hashtableToCheck' au
	}
